...
Ada tiga aksioma dasar yang melandasi penggunaan fungsi ini: 1. \textbf{Aksioma Signal-to-Noise:} Investor tidak melihat keuntungan (\(\mu\)) secara terisolasi, melainkan selalu membandingkannya dengan derau/risiko (\(\sigma\)). Rasio \(\mu/\sigma\) adalah penggerak utama keputusan. 2. \textbf{Aksioma Saturasi:} Nafsu terhadap keuntungan tidak tumbuh linear tanpa batas. Ada titik di mana tambahan keuntungan tidak lagi menambah ``nafsu'' secara signifikan (titik jenuh/saturasi). 3. \textbf{Aksioma Batas:} Parameter \(\lambda\) harus berada di antara 0 (fokus total pada risiko) dan 1 (fokus total pada return).
...
\textbf{Pasar Netral} & \(\mu = 0\) & \(\lambda = 0.5\) & Keseimbangan antara keamanan dan cuan. \\
\textbf{Pasar Panic} & \(\mu \ll 0\) & \(\lambda \to 0\) & Memilih aset paling aman (varians terendah). \\
\end{longtable}
...
\hypertarget{physical-insight-efek-termostat-kuantum}{%
\subsection{Physical Insight: Efek ``Termostat'' Kuantum}\label{physical-insight-efek-termostat-kuantum}}

Bayangkan \(\lambda\) sebagai sebuah \textbf{termostat} otomatis. 1. Saat pasar ``panas'' (return tinggi), termostat membuka katup return lebar-lebar (\(\lambda \uparrow\)). 2. Saat pasar ``dingin'' (risiko tinggi), termostat menutup katup return dan menyalakan mode aman (\(\lambda \downarrow\)).

\textbf{Hubungan dengan Ising Hamiltonian:} Parameter \(\lambda\) endogen ini secara otomatis mengatur ulang kekuatan \textbf{Field Lokal (\(h_i\))} terhadap \textbf{Kopling (\(J_{ij}\))}. Tanpa campur tangan manusia, sirkuit kuantum Anda akan menyesuaikan ``arah ukurannya'' sendiri sesuai dengan iklim data yang masuk.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{kesimpulan}{%
\subsection{Kesimpulan}\label{kesimpulan-b}}

Dengan menggunakan fungsi sigmoid: - Kita menghindari bias subjektif dalam memilih \(\lambda\). - Kita memberikan sifat \textbf{non-linear} yang lebih mirip dengan perilaku asli manusia. - Kita memastikan bahwa sistem kuantum selalu memiliki skala energi yang konsisten.

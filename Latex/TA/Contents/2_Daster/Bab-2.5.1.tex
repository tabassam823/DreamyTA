...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Simbolisme gerbang Pauli dan gerbang Hadamard pada sirkuit kuantum yang merepresentasikan transformasi uniter dasar $X$, $Y$, $Z$, dan $H$ pada satu qubit.}
...
\end{split}
\end{equation}
Penurunan rumus penuh dari persamaan \eqref{eq:gerbang_pauli_basis} menjadi persamaan \eqref{eq:gerbang_rotasi_spesifik} dapat ditinjau pada Lampiran \ref{app:su2_taylor}. Parameter sudut rotasi $\theta$ tersebut menjadi parameter yang dioptimasi bagi algoritma klasik untuk mencari konfigurasi energi terendah dari Hamiltonian Ising. Representasi skematik dari gerbang rotasi satu qubit divisualisasikan pada Gambar \ref{fig:gerbang_rotasi}, di mana kotak berlabel $R_x(\theta)$, $R_y(\theta)$, dan $R_z(\theta)$ menunjukkan masing-masing operasi transformasi uniter yang bekerja pada alur data qubit.
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[scale=1.3, >=stealth]
        % Gerbang R_x

...
Secara etimologi, \textit{barren plateau} berasal dari kata barren yang berarti tandus atau tidak produktif dan plateau yang berarti dataran tinggi yang relatif datar. Karakteristik utama dari \textit{barren plateau} dapat diamati melalui perilaku varians gradien terhadap ukuran sistem kuantum~\cite{shannon_mathematical_1948}. Ketika jumlah qubit dan kedalaman sirkuit meningkat, varians gradien cenderung menurun secara eksponensial sehingga nilai gradien yang terukur semakin sulit dibedakan dari fluktuasi statistik dan derau pengukuran~\cite{serafini_symplectic_2003}. Secara umum, hubungan tersebut dapat dituliskan sebagai
\begin{equation}
    \text{Var}[\partial_\theta E] \approx \frac{C}{2^{N+L}}
\end{equation}
dengan $C$ merupakan konstanta yang bergantung pada Hamiltonian sistem dan struktur Hamiltonian yang digunakan. Persamaan tersebut menunjukkan bahwa peningkatan jumlah qubit maupun kedalaman sirkuit menyebabkan sinyal gradien menyusut secara eksponensial~\cite{ortiz_marrero_entanglement-induced_2021}. Dalam batas tertentu, nilai gradien menjadi sangat kecil sehingga hampir tidak memberikan informasi yang berguna bagi proses optimasi~\cite{barron_minimum_1991}.

\subsection{Entropi Von Neumann}
Entropi Von Neumann digunakan untuk mengukur tingkat ketidakpastian dan keterbelitan dalam sistem kuantum. Konsep ini merupakan generalisasi dari Entropi Shannon pada teori informasi klasik~\cite{shin_estimating_2024}. Untuk suatu distribusi probabilitas diskrit $p_i$, Entropi Shannon didefinisikan sebagai
\begin{equation}

...
Secara komputasional, data historis diperoleh melalui antarmuka pemrograman aplikasi atau \textit{Application Programming Interface} (API) Yahoo Finance menggunakan pustaka \texttt{yfinance} pada bahasa pemrograman Python. Pengunduhan data dilakukan secara otomatis untuk memperoleh harga penutupan harian setiap saham serta indeks pembanding secara konsisten. Setelah proses ekstraksi selesai, data dibersihkan dari nilai yang hilang (\textit{missing values}) dan diurutkan berdasarkan indeks waktu guna menjaga konsistensi deret waktu (\textit{time series}) sebelum digunakan pada tahap analisis berikutnya. Implementasi proses pengambilan dan pembersihan data tersebut disajikan pada Daftar Kode \ref{lst:data_downloader}.

\begin{lstlisting}[language=Python, caption={Skrip ekstraksi data harga historis saham menggunakan API \texttt{yfinance}}, label={lst:data_downloader}]
...
Selisih antara kedua ekspansi tersebut merepresentasikan galat aproksimasi yang muncul akibat penggunaan imbal hasil logaritmik berbobot sebagai pengganti imbal hasil logaritmik portofolio yang sebenarnya. Galat tersebut dapat dituliskan sebagai
...
\text{Var}(R).
\end{equation}
Sehingga, hubungan antara imbal hasil logaritmik portofolio yang sebenarnya dan aproksimasi berbobotnya dapat dinyatakan sebagai
\begin{equation}
r_{true}
\approx
\hat{r}_p
...
\end{equation}
yang menunjukkan bahwa aproksimasi berbasis imbal hasil logaritmik mengandung koreksi sebesar setengah varians portofolio. Oleh karena itu, ekspektasi imbal hasil dalam model optimasi portofolio dihitung menggunakan imbal hasil sederhana agar tetap konsisten dengan sifat linear formulasi Markowitz.

Meskipun ekspektasi imbal hasil dihitung menggunakan imbal hasil sederhana, estimasi matriks kovariansi dalam penelitian ini dilakukan menggunakan imbal hasil logaritmik. Pemilihan tersebut didasarkan pada sifat imbal hasil logaritmik yang lebih stabil secara statistik serta memiliki distribusi yang cenderung lebih mendekati asumsi normalitas dibandingkan imbal hasil sederhana. Selain itu, untuk nilai imbal hasil harian yang relatif kecil, perbedaan antara kedua representasi tersebut sangat kecil sehingga tidak menghasilkan perubahan yang signifikan terhadap estimasi risiko portofolio. Berdasarkan Persamaan \ref{eq:eror_returns}, hubungan antara imbal hasil sederhana dan imbal hasil logaritmik dapat dituliskan secara aproksimatif sebagai
\begin{equation}
...
r_i \approx R_i.
\end{equation}
Akibatnya, kovariansi yang dihitung menggunakan imbal hasil logaritmik akan mendekati kovariansi yang dihitung menggunakan imbal hasil sederhana. Secara eksplisit,
\begin{equation*}
\begin{aligned}
\text{Cov}(r_A,r_B)
&\approx
...
\text{Cov}(R_A,R_B).
\end{equation}
Hasil ini menunjukkan bahwa penggunaan imbal hasil logaritmik untuk estimasi matriks kovariansi tetap memberikan representasi risiko yang konsisten terhadap penggunaan imbal hasil sederhana, terutama pada data saham harian dengan tingkat volatilitas yang relatif rendah.


Transformasi data harga saham menjadi imbal hasil sederhana dan imbal hasil logaritmik diimplementasikan menggunakan pustaka \texttt{NumPy} dan \texttt{Pandas}. Fungsi yang dikembangkan menerima masukan berupa deret harga historis dan menghasilkan deret imbal hasil sesuai dengan formulasi matematis yang telah dijelaskan sebelumnya. Selain itu, dilakukan pula perhitungan nilai ekspektasi imbal hasil sederhana sebagai parameter pertumbuhan aset pada setiap jendela pelatihan. Untuk keperluan analisis risiko, imbal hasil logaritmik dihitung menggunakan transformasi logaritma natural terhadap rasio harga berturut-turut. Sementara itu, estimasi ekspektasi imbal hasil logaritmik diperoleh melalui pendekatan koreksi volatilitas (\textit{volatility drag}) yang dinyatakan sebagai selisih antara rata-rata imbal hasil sederhana dan setengah variansnya. Pendekatan ini digunakan untuk menjaga konsistensi antara estimasi pertumbuhan geometrik dan karakteristik fluktuasi harga yang diamati pada data historis. Implementasi lengkap dari proses transformasi imbal hasil dan perhitungan parameter statistik tersebut ditunjukkan pada Daftar Kode \ref{lst:calc_returns}.
...
Sebagai ilustrasi penerapan formulasi yang telah dijelaskan sebelumnya, dilakukan perhitungan numerik menggunakan data historis saham BBCA pada periode observasi pertama. Berdasarkan Tabel \ref{tab:daily_prices_merged}, harga penutupan meningkat dari 4515,50 menjadi 4869,32 sehingga diperoleh imbal hasil sederhana dan imbal hasil logaritmik harian sebagai berikut
\begin{equation*}
\begin{aligned}
R_{BBCA,1}
&=
...
\end{aligned}
\end{equation*}
Nilai varians dan kovariansi selanjutnya dihitung menggunakan data imbal hasil logaritmik pada jendela pelatihan yang terdiri atas 126 hari perdagangan. Untuk dua aset $i$ dan $j$, kovariansi sampel didefinisikan sebagai
\begin{equation}
\begin{aligned}
\sigma_{ij}
&=
...
\end{aligned}
\end{equation}
dengan pendekatan pada baris kedua mengikuti hasil analisis pada Subsection \ref{subsec:4_1_1_rumus} yang menunjukkan bahwa kovariansi imbal hasil logaritmik dan imbal hasil sederhana bernilai hampir identik untuk data imbal hasil harian.
...
\subsection{Parameter penghindaran risiko Endogen}
\label{subsec:4_1_3_risk_aversion}

Dalam model optimasi portofolio Markowitz, parameter penghindaran risiko ($\gamma$) umumnya diperlakukan sebagai konstanta eksogen yang nilainya ditentukan sebelum proses optimasi dilakukan. Berbeda dengan pendekatan tersebut, penelitian ini menggunakan parameter penghindaran risiko yang dihitung secara endogen berdasarkan karakteristik data pasar pada setiap jendela pelatihan. Dengan demikian, tingkat penalti risiko dapat berubah secara adaptif mengikuti kondisi pasar yang diamati. Nilai $\gamma$ ditentukan melalui fungsi sigmoid yang dibangun dari perbandingan antara rata-rata ekspektasi imbal hasil logaritmik ($\bar{\mu}_l$) dan rata-rata volatilitas logaritmik ($\bar{\sigma}_l$), yaitu
\begin{equation}
\gamma =
\frac{1}

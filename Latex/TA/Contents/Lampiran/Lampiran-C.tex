\chapter{Analisis Perbandingan Imbal Hasil Sederhana dan Logaritmik}
\label{appendix:returns_comparison}

Imbal hasil sederhana ($R_t$) didefinisikan sebagai perubahan persentase harga relatif:
\begin{equation}
    R_t = \frac{P_t}{P_{t-1}} - 1
\end{equation}
Imbal hasil logaritmik ($r_t$) didefinisikan menggunakan logaritma natural:
\begin{equation}
    r_t = \ln\left(\frac{P_t}{P_{t-1}}\right) = \ln(1 + R_t)
\end{equation}

Melalui ekspansi deret Taylor orde kedua untuk fungsi $\ln(1+x) \approx x - \frac{x^2}{2}$, kita dapat melihat hubungan antara ekspektasi kedua jenis imbal hasil tersebut:
\begin{equation}
...
\end{equation}
Sehingga kovariansi antara aset $A$ dan $B$ memenuhi:
\begin{equation}
    Cov(r_A, r_B) \approx Cov(R_A, R_B)
\end{equation}
Secara lebih rinci, melalui ekspansi Taylor bilinear:
\begin{equation}

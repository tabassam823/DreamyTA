...
\Procedure{BuildHamiltonian}{$\mu, \Sigma, \gamma, K, \lambda$}
    ...
    \State $C \gets \frac{1}{2} \sum Q_{ii} + \frac{1}{4} \sum Q_{ij} + \lambda K^2$ \Comment{Konstanta Energi}
    \State $H \gets \sum h_i Z_i + \sum J_{ij} Z_i Z_j + C \cdot I$ \Comment{Konstruksi Matriks Hamiltonian}
    \State \Return $H$
\EndProcedure
\end{algorithmic}
\end{algorithm}

...
def find_optimal_lr_spsa(H, n_qubits, curr_date, ne_bitstring=None, K=2, test_iters=30, file_config=None):
    """Mencari Learning Rate (a) optimal dan mengekspor ke CSV dengan tanggal."""
    if file_config is None:
        file_config = {'pencarian_lr': 'hasil_pencarian_lr.csv'}
    ...
    if ne_bitstring is not None:
        ...
        init_p = init_w.flatten() + rng.normal(0, 1e-3, n_params)
    else:
        init_p = rng.uniform(0, 2 * np.pi, n_params)
        
    test_a_values = np.logspace(-3, 0, 8)   # dikurangi dari 15 kandidat untuk efisiensi
    final_energies = []
    ...
    lr_df.to_csv(file_config['pencarian_lr'], mode='a', header=not os.path.exists(file_config['pencarian_lr']), index=False)

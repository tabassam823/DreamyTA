...
\begin{frame}{Bab 3.1 - Alur Penelitian}
    ...
    \resizebox{0.85\textwidth}{!}{
        \begin{tikzpicture}[node distance=1.3cm, every node/.style={fill=white, font=\footnotesize}, align=center]
            \tikzset{
                startstop/.style={ellipse, minimum width=2.5cm, minimum height=0.6cm, text centered, draw=black, fill=white},
                process/.style={rectangle, minimum width=3.2cm, minimum height=0.8cm, text centered, draw=black, fill=white},
                decision/.style={diamond, minimum width=2.5cm, minimum height=1cm, text centered, draw=black, fill=white, aspect=2},
                connector/.style={circle, draw=black, minimum size=0.5cm, text centered, inner sep=2pt},
                arrow/.style={thick,->,>=stealth}
            }
            \node (start) [startstop] {Mulai};
            \node (data) [process, below of=start, yshift=-0.5cm] {Akuisisi Data dan \\ Setup Jendela Waktu};
            \node (param) [process, below of=data, yshift=-0.5cm] {Hitung $\mu$, $\Sigma$ dan \\ \textit{Risk Aversion} endogen ($\gamma$)};
            \node (hamil) [process, below of=param, yshift=-0.5cm] {Formulasi QUBO dan \\ Hamiltonian ($h_i, J_{ij}$)};
            ...
            \node (vqe_init) [process, below of=c1b, yshift=-0.2cm] {Inisialisasi State \\ $|0\dots0\rangle \to |\text{bitstring}_{NE}\rangle$};
            \node (spsa) [process, below of=vqe_init, yshift=-0.5cm] {Optimasi SPSA};
            \node (conv) [decision, below of=spsa, yshift=-0.5cm] {Konvergen?};
            \node (layer_eval) [process, below of=conv, yshift=-0.6cm] {Simpan $E_d$ dan \\ Tambah \textit{Depth}};
            \node (layer_dec) [decision, below of=layer_eval, yshift=-0.5cm] {$d < D_{max}$?};
            \node (best_state) [process, below of=layer_dec, yshift=-0.7cm] {Pilih $E_{min}$};
            ...
            \node (eval) [process, below of=rebal, yshift=-1cm] {Kalkulasi Metrik \\ Akhir};
            ...
            \draw [arrow] (c1b) -- (vqe_init); \draw [arrow] (vqe_init) -- (spsa); \draw [arrow] (spsa) -- (conv); \draw [arrow] (conv) -- node[anchor=east] {Ya} (layer_eval);
            \draw [arrow] (layer_eval) -- (layer_dec); \draw [arrow] (layer_dec) -- node[anchor=east] {Tidak} (best_state); \draw [arrow] (best_state) -- (c2a);

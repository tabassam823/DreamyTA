...
* **Variance ($\sigma^2$):**
  $$ \sigma_{i}^2 = \frac{1}{n-1} \sum_{t=1}^n (R_{i,t} - \bar{R}_i)^2 $$
...
* **Expected Return ($\bar{r}$):**
  $$ \bar{r}_i = \frac{1}{n} \sum_{t=1}^n r_{i,t} $$
* **Variance ($\sigma^2$):**
  $$ \sigma_{i}^2 = \frac{1}{n-1} \sum_{t=1}^n (r_{i,t} - \bar{r}_{i})^2 $$
* **Covariance ($\sigma_{ij}$):**
  $$ \sigma_{ij} = \frac{1}{n-1} \sum_{t=1}^n (r_{i,t} - \bar{r}_i)(r_{j,t} - \bar{r}_j) $$
:::
::::
...
* $P_{i,t}$: Harga pada waktu $t$ untuk aset ke-$i$
:::
::::
:::::
...
$$f(\mathbf{x}) = \sum_{i,j} \sigma_{ij} \frac{x_i}{K} \frac{x_j}{K} - \lambda \sum_i \mu_i \frac{x_i}{K}$$

$$\therefore f(\mathbf{x}) = \sum_{i,j} \sigma_{ij} \frac{x_i x_j}{K^2} - \lambda \sum_i \mu_i \frac{x_i}{K}$$

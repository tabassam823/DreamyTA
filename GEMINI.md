...
Semua interaksi dan pembuatan konten dalam workspace ini (terutama di folder `/Latex/TA`) WAJIB mengikuti protokol berikut:
...
3. **Background & Motivation:** Jelaskan urgensi masalah berdasarkan gap dalam literatur atau kebutuhan sistem fisik/finansial secara fundamental.
4. **Theoretical Framework:** Bangun argumen dari aksioma dan postulat dasar secara deduktif.
...
6. **Mathematical Rigor:** Sajikan derivasi langkah-demi-langkah dengan presisi tinggi. Pastikan setiap transisi variabel memiliki justifikasi fisik atau matematis yang eksplisit.
7. **Technical Synthesis:** Saat membahas sistem kuantum (seperti VQE atau Qubits), gunakan analogi teknis sirkuit (gates, noise, coherence) yang setara dengan representasi hardware.
8. **Data Representation:** Fokus pada interpretasi hasil melalui deskripsi grafis, konvergensi energi, atau metrik performa yang terukur.
9. **Visual Integration & Figure Descriptions:** Jika suatu konsep kompleks memerlukan visualisasi, AI wajib memberikan saran gambar (deskripsi elemen visual dan usulan caption). Gambar tersebut harus dirujuk secara eksplisit di dalam paragraf (misal: "Figure 1 mengilustrasikan...") dan dijelaskan fungsinya dalam mendukung argumen teknis. Deskripsi di dalam teks harus mencakup detail kritis yang diharapkan muncul pada gambar agar tercipta koherensi antara narasi dan visual.
...
- **Referensi:** Prioritaskan data dari `/paper`, `/notes`, dan `/Latex/PakGagus_...` untuk menjaga kontinuitas riset.
...
## 4. Prioritas Folder
- Fokus utama penulisan adalah draf Tugas Akhir pada direktori `/Latex/TA/`.
- Jangan melakukan perubahan pada file `.pdf` atau struktur `.obsidian`.

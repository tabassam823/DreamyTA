...
Dokumen ini menyajikan kerangka kerja komprehensif yang menggabungkan formalisme *Exact Potential Game* dengan *Classical Mutual Information* (CMI) untuk mentransformasikan masalah seleksi portofolio Markowitz ke dalam Hamiltonian Ising yang siap dioptimasi menggunakan algoritma *Variational Quantum Eigensolver* (VQE).
...
## 4. Integrasi Classical Mutual Information (CMI) dan Dinamika Strategi
...
Interaksi antar aset dalam matriks kovariansi $\Sigma$ seringkali gagal menangkap ketergantungan non-linear dan sinkronisasi informasional yang kompleks. Oleh karena itu, elemen interaksi $\sigma_{ij}$ diperluas menggunakan *Classical Mutual Information* (CMI) yang dilambangkan dengan $I(i:j)$. CMI mengukur jumlah informasi (dalam satuan *bits*) yang dibagikan antar dua aset berdasarkan distribusi strategi bersama (*joint distribution*). Secara formal, distribusi strategi bersama untuk pasangan aset $(i,j)$ didefinisikan sebagai:
$$P(S_i = s, S_j = s') = \frac{1}{T} \sum_{t=1}^T \mathbb{I}(S_{i,t} = s) \cdot \mathbb{I}(S_{j,t} = s'), \quad s, s' \in \{u, d\} \quad (14)$$

Berdasarkan probabilitas bersama tersebut, nilai CMI diturunkan menggunakan formulasi entropi Shannon untuk mengukur derajat sinkronisasi strategi:
$$I(i:j) = \sum_{s \in \{u,d\}} \sum_{s' \in \{u,d\}} P(S_i=s, S_j=s') \log_2 \frac{P(S_i=s, S_j=s')}{P(S_i=s) \cdot P(S_j=s')} \quad (15)$$

Elemen kovariansi yang diperkaya ($\tilde{\sigma}_{ij}$) dirumuskan sebagai modifikasi dari kovariansi standar melalui inkorporasi penalti informasional. Karena $I(i:j)$ adalah besaran adimensional sementara $\sigma_{ij}$ memiliki dimensi unit imbal hasil kuadrat, diperlukan koefisien kopling dimensional $\xi^{CMI}$ untuk memastikan kesetaraan skala:
$$\tilde{\sigma}_{ij} = \sigma_{ij} [1 + \xi^{CMI} \cdot I(i:j)] \quad (16)$$

> Justifikasi Penskalaan Analitis $\xi^{CMI}$:
> Untuk menjamin bahwa suku penalti informasional berada pada skala yang sebanding dengan risiko varians tanpa memerlukan validasi silang (*cross-validation*), $\xi^{CMI}$ didefinisikan sebagai rasio rata-rata magnitudo kovariansi terhadap rata-rata CMI dalam semesta aset:
> $$\xi^{CMI} = \frac{\bar{\sigma}}{\bar{I}^{CMI}} = \frac{\sum_{i < j} |\sigma_{ij}|}{\sum_{i < j} I(i:j)} \quad (17)$$

Dengan mensubstitusikan $\tilde{\mu}_i$ dan $\tilde{\sigma}_{ij}$ ke dalam kerangka kerja *Potential Game*, Fungsi Potensial yang telah diperkaya secara strategis dan informasional menjadi:
$$\Phi_{enriched}(\mathbf{x}) = \sum_{i=1}^N \left( \tilde{\mu}_i - \frac{\gamma}{2} \sigma_{ii} \right) x_i - \gamma \sum_{i < j} \tilde{\sigma}_{ij} x_i x_j \quad (18)$$


## 5. Pemetaan ke Domain Quadratic Unconstrained Binary Optimization (QUBO)
### 5.1. Formulasi Pure QUBO
Transisi dari kerangka kerja *Potential Game* ke komputasi kuantum memerlukan restrukturisasi fungsi objektif dari maksimasi potensial menjadi minimasi energi biaya. Masalah *Quadratic Unconstrained Binary Optimization* (QUBO) didefinisikan sebagai pencarian vektor biner $\mathbf{x}^*$ yang meminimalkan fungsi kuadratik dalam ruang diskrit $\mathbb{B}^N$. Secara formal, biaya murni (*pure cost*) diturunkan sebagai negatif dari Fungsi Potensial yang telah diperkaya pada Persamaan (18):
$$H_{pure}(\mathbf{x}) = -\Phi_{enriched}(\mathbf{x}) = \sum_{i=1}^N \left( \frac{\gamma}{2} \sigma_{ii} - \tilde{\mu}_i \right) x_i + \gamma \sum_{i < j} \tilde{\sigma}_{ij} x_i x_j \quad (19)$$

Dalam representasi matriks, fungsi ini dinyatakan dalam bentuk kuadratik $\mathbf{x}^T Q \mathbf{x}$, di mana $Q \in \mathbb{R}^{N \times N}$ adalah matriks QUBO simetris. Dengan memanfaatkan sifat idempotensi $x_i^2 = x_i$, koefisien linier pada Persamaan (19) dipetakan langsung ke elemen diagonal matriks $Q$:
$$Q_{ii} = \frac{\gamma}{2} \sigma_{ii} - \tilde{\mu}_i, \quad Q_{ij} = \frac{\gamma}{2} \tilde{\sigma}_{ij} \quad (20)$$
...
$$P(\mathbf{x}) = \lambda \left( \sum_{i=1}^N x_i - K \right)^2 \quad (21)$$
di mana $\lambda \in \mathbb{R}^+$ adalah pengali Lagrange (*penalty factor*) yang besarnya harus melampaui rentang nilai pada biaya murni guna menjamin kepatuhan kendala. Ekspansi aljabar dari Persamaan (21) menghasilkan:
$$P(\mathbf{x}) = \lambda \left[ \sum_{i=1}^N x_i^2 + 2\sum_{i < j} x_i x_j - 2K \sum_{i=1}^N x_i + K^2 \right] \quad (22)$$

Dengan mensubstitusikan kembali sifat $x_i^2 = x_i$, fungsi penalti dikonsolidasikan ke dalam format QUBO standar:
$$P(\mathbf{x}) = \lambda (1 - 2K) \sum_{i=1}^N x_i + 2\lambda \sum_{i < j} x_i x_j + \text{const} \quad (23)$$
...
$$Q_{ii}^{total} = \left( \frac{\gamma}{2} \sigma_{ii} - \tilde{\mu}_i \right) + \lambda(1 - 2K) \quad (24)$$
$$Q_{ij}^{total} = \gamma \tilde{\sigma}_{ij} + 2\lambda \quad (25)$$
...
$$x_i \mapsto \frac{1 - \hat{Z}_i}{2} \quad (26)$$
Substitusi operator ini ke dalam fungsi objektif QUBO pada Persamaan (19) dan (23) akan mengubah lanskap energi biner menjadi operator Hamiltonian kuantum. Proses ini menyebabkan munculnya interaksi antar-qubit ($J_{ij}$) yang merepresentasikan korelasi strategis dan medan magnet lokal ($h_i$) yang merepresentasikan bias individu setiap aset. Konstanta energi yang muncul dari ekspansi aljabar dapat diabaikan dalam proses optimasi karena tidak mengubah lokasi *Ground State*.

### 6.2. Ekstraksi Parameter Kopling ($J_{ij}$) dan Medan Lokal ($h_i$)
Hamiltonian Ising final dirumuskan sebagai operator linier dalam ruang Hilbert $N$-qubit, yang terdiri dari jumlahan interaksi *pairwise* dan medan *transverse*. Berdasarkan substitusi Persamaan (26) ke dalam profil energi total $H_{total}$, parameter fisik Hamiltonian didefinisikan melalui koefisien matriks QUBO $Q^{total}$. Kekuatan kopling antar-qubit $J_{ij}$ diturunkan dari elemen *off-diagonal* yang mencakup penalti interaksi dan CMI, sementara medan lokal $h_i$ merupakan akumulasi dari kontribusi diagonal dan normalisasi interaksi.

Secara formal, parameter Hamiltonian Ising total didefinisikan sebagai:
$$J_{ij}^{total} = \frac{Q_{ij}^{total}}{4} = \frac{\gamma \tilde{\sigma}_{ij} + 2\lambda}{4} \quad (27)$$
$$h_i^{total} = -\frac{Q_{ii}^{total}}{2} - \sum_{j \neq i} \frac{Q_{ij}^{total}}{4} \quad (28)$$
dengan substitusi nilai $Q_{ii}^{total}$ dari Persamaan (24) dan (25), diperoleh ekspresi eksplisit:
$$h_i^{total} = -\frac{1}{2} \left[ \left( \frac{\gamma}{2} \sigma_{ii} - \tilde{\mu}_i \right) + \lambda(1 - 2K) \right] - \sum_{j \neq i} \frac{\gamma \tilde{\sigma}_{ij} + 2\lambda}{4} \quad (29)$$
Hamiltonian kuantum final $\hat{H}_{Ising}$ yang akan dieksekusi pada algoritma VQE dinyatakan sebagai:
$$\hat{H}_{Ising} = \sum_{i < j} J_{ij}^{total} (\hat{Z}_i \otimes \hat{Z}_j) + \sum_{i=1}^N h_i^{total} \hat{Z}_i \quad (30)$$
Operator pada Persamaan (30) menyatukan seluruh dimensi riset: utilitas *Game Theory*, diversifikasi CMI, dan kendala kardinalitas ke dalam satu operator energi tunggal yang keadaan dasarnya (*Ground State*) merupakan solusi portofolio optimal.
...
$$|\psi(\theta)\rangle = U(\theta) |0\rangle^{\otimes N} \quad (31)$$
...
$$E(\theta) = \langle \psi(\theta) | \hat{H}_{Ising} | \psi(\theta) \rangle \quad (32)$$
